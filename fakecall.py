...
import re, sys, os, json, requests, random, time, rich
...
#-----------------[ INI-LOGO-KONTOL ]---------------#
def logo():
	banner = f"[white][[yellow]•[white]] Status	[red]:[green] Online\n[white][[yellow]•[white]] Admin		[red]:[green] TOUFIQ AHMED [white] "
	cetak(nel(banner, style='cyan', title='[blue]Informasi[white]'))
	banner2 = f"[white][[cyan]01[white]]. Fake Call's\n[[cyan]02[white]]. Upcoming\n[[cyan]03[white]]. Back ([yellow] out[white] )"
	cetak(nel(banner2, style='cyan'))
...
class Main:
	...
	def main(self):
		global nomor
		logo()
		sanz = input(f"  {x}[{h}?{x}] Input Pilihan : ")
		if sanz == "1" or sanz == "01":
			asuh = '[white][[green]•[white]] Disarankan Untuk Spam Mantan'
			cetak(nel(asuh, style='cyan'))
			print(f"\r  {x}[{b}•{x}] Contoh {b}: {h}+8801812xxxxxx")
			nomor = input(f"  {x}[{b}•{x}] Number {b}: {k}+880").replace("+880","")
			bi = random.choice([m,k,h,u])
			print('')
			sleep(1)
			if nomor == "":
				pass
				...
				while True:
					sxp_sms()
					sxp_call()
					asu = random.choice([b,u,i,x])
					jalan(f'{asu}')
					self.wait(3) # <- waktu delay
					sys.stdout.write("\r                           ");sys.stdout.write("\r")
					sleep(1)

		elif sanz == "2" or sanz == "02":
			print(f"\n     {red}! {white}Follow Facebook TOUFIQ AHMED")
			# Ganti link fb nya v
...
class sxp_sms:
	...
	def sms_otp_6(self, no):
		aink_sanz = random.choice(
						[
							"buka blokan lah bisi riset hp maneh te sesuai perjanjian",
							"Hallo Bangsad",
							"Hallo Sayang",
							"Hallo Ripper",
							"Hallo Ngab"
	...
	def main(self):
		...
		jalan(f'  {x}[{b}✓{x}] {h}SPAM SMS BERHASIL.....!!{x}')
...
class sxp_wa:
	...
	def main(self):
		...
		self.wa_otp_5(nomor)
		self.wa_otp_6(nomor)
		jalan(f'  {x}[{b}✓{x}]{h} SPAM Call Successful......!!{x}')
